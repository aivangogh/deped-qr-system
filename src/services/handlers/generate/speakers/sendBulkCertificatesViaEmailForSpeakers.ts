    ...
    for (const speaker of speakers) {
      ...
      try {
        const mailOptions: SendMailOptions = {
          ...
          to: speaker.email, // Use the email address if it exists
          subject: 'Certificate for ' + speaker.speaker,
          text: `Dear ${speaker.speaker},
...
<p>Congratulations on your well-deserved accomplishment. Should you require any additional documentation or further assistance, please do not hesitate to contact us.</p>`,
          attachments: [
            {
              filename: `${speaker.speaker?.toLocaleLowerCase()}-certificate.docx`,
              content: certificates.find(
                (certificate) =>
                  certificate.fileName ===
                  `${speaker.speaker?.toLocaleLowerCase()}-certificate.docx`
              )?.buffer,
            },
          ],
        };
